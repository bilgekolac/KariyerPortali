var initTable1 = function () {
    ...
    table.dataTable({
        ...
        "bServerSide": true,
        "bProcessing": true,
        "sAjaxSource": "/FormInfo/AjaxHandler",
             ...
             },
             {
                 ...
                 'render': function (data, type, row) {
                     return '<div class="btn-group"><button class="btn btn-xs green dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false">Eylemler<i class="fa fa-angle-down"></i></button>'
                         + '<ul class="dropdown-menu" role="menu"><li><a href="/FormInfo/Edit/' + row[0] + '"><i class="icon-note"></i> Düzenle</a></li><li><a href="/FormInfo/Details/' + row[0] + '"><i class="icon-list"></i> Detaylar</a></li><li>'
                         + '<a href="/FormInfo/Delete/' + row[0] + '" onclick="if (!confirm(\'Bu kaydı silmek istediğinize emin misiniz? Bu işlem geri alınamaz.\')) return false;"><i class="icon-ban"></i> Sil</a></li></ul></div>';
                 }
    ...
    var tableWrapper = jQuery('#allFormInfoTable_wrapper');
